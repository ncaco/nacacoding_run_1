...
try {
    ...
    $userMe = Invoke-RestMethod -Uri "http://localhost:8080/api/v1/users/me" `
        -Method GET `
        -Headers $headers
    
    Write-Host "✅ 사용자 정보 조회 성공!" -ForegroundColor Green
    Write-Host "   사용자명: $($userMe.data.username)" -ForegroundColor Gray
    Write-Host "   역할: $($userMe.data.role)" -ForegroundColor Gray
} catch {
    Write-Host "❌ 사용자 정보 조회 실패: $($_.Exception.Message)" -ForegroundColor Red
}

# 3. 사용자 목록 조회
Write-Host "`n[3] 사용자 목록 조회 테스트..." -ForegroundColor Yellow
try {
    ...
    Write-Host "   총 사용자 수: $($users.data.Count)명" -ForegroundColor Gray
...
# 4. 관리자 대시보드 테스트 (ADMIN 권한 필요)
Write-Host "`n[4] 관리자 대시보드 테스트..." -ForegroundColor Yellow
try {
    $admin = Invoke-RestMethod -Uri "http://localhost:8080/api/v1/admin/dashboard" `
        -Method GET `
        -Headers $headers
    
    Write-Host "✅ 관리자 대시보드 접근 성공!" -ForegroundColor Green
    Write-Host "   메시지: $($admin.data.message)" -ForegroundColor Gray
} catch {
...
try {
    ...
    try {
        $testAfterLogout = Invoke-RestMethod -Uri "http://localhost:8080/api/v1/users/me" `
            -Method GET `
            -Headers $headers
