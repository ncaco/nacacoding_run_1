...
interface UserRoleListProps {
  ...
  onManageMenuPermissions?: (userRole: UserRole) => void;
}

function UserRoleItem({ userRole, onEdit, onDelete, onToggleEnabled }: { userRole: UserRole; onEdit?: (userRole: UserRole) => void; onDelete?: (userRole: UserRole) => void; onToggleEnabled?: (userRole: UserRole, enabled: boolean) => void }) {
      ...
      <div className="flex flex-col gap-2 pt-3 border-t border-slate-100 dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
        ...
        {onToggleEnabled && (
          <div className="flex items-center gap-2">
            <ToggleSwitch
              enabled={userRole.enabled ?? true}
              onToggle={(enabled) => onToggleEnabled(userRole, enabled)}
              size="sm"
            />
            <span className={`text-xs font-medium ${userRole.enabled ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-500 dark:text-slate-300'}`}>
              {userRole.enabled ? '활성' : '비활성'}
            </span>
          </div>
        )}
        ...
        <div className="flex items-center gap-1.5 flex-wrap self-end ml-auto">
          <button
            onClick={() => router.push(`/admin/user-roles/${userRole.id}/menu-permissions`)}
