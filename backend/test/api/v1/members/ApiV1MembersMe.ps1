# API 테스트: 현재 사용자(MEMBER) 정보 조회
# 사용법: .\ApiV1MembersMe.ps1

# 공통 설정 로드
. $PSScriptRoot\..\..\..\_config.ps1

Write-Host "=== 현재 사용자(MEMBER) 정보 조회 ===" -ForegroundColor Cyan
...
try {
    $response = Invoke-RestMethod -Uri (Get-ApiUrl "members/me") `
        -Method GET `
        -Headers $headers
